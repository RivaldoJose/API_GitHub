...
namespace PS_APIGit.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                if (!await ExisteRepositorioNoGit(model.Username))
                {
                    ModelState.AddModelError("Username", "Este nome de usuario não foi encontrado no github. Por favor, tente se cadastrar no site.");
                    return View(model);
                }
                else
                {
                    string novoEmail = model.Username + "@teste.com";
                    var user = new ApplicationUser { UserName = model.Username, Email = novoEmail };
        ...
        private async Task<bool> ExisteRepositorioNoGit(string username)
        {
            try
            {
